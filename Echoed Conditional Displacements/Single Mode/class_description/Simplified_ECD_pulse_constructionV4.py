...
'''
V3: Wanted to also output pulses in gaussian parameters form for ease of conversion to qick pulses
    No detuning of qubit pulse
    No wait time between ecd pulses’
    Code is designed to have multiple qubit rotations between pulses but cavity pulse code does not take that into account?
    No padding 

V4: make sure that qubit_dac_gauss and cavity_dac_gauss are alligned
...
def conditional_displacement(
    ...
    def construct_CD(alpha, tw, r, r0, r1, r2, buf=0):
        '''
        returns a pulse in normal time domain but also in terms of a list with  elements [amp, sigma]; if amp = 0 sigma will be wait time 
        '''
        ...
        dr_, di_ = storage.displace.make_wave(pad=False)
        d_ = storage.displace.unit_amp * (dr + 1j * di)
        cavity_dac_gauss = np.array([[alpha * np.max(np.abs(d_)) * np.exp(1j * phase), storage.displace.sigma],   # note that d = dr since di = 0
                            [0, tw],
                            [r0 * alpha  * np.max(np.abs(d_)) * np.exp(1j * (phase + np.pi)),  storage.displace.sigma],
                            [0, buf],
                            [0, len(p)], 
                            [0, buf],
                            [r1 * alpha  * np.max(np.abs(d_)) * np.exp(1j * (phase + np.pi)), storage.displace.sigma], 
                            [0, tw],
                            [r2 * alpha  * np.max(np.abs(d_)) * np.exp(1j * (phase)), storage.displace.sigma]
                            ])
        ...
        qubit_dac_gauss = np.array([[0, len(d)],
                                    [0, tw],
                                    [0, len(d)],
                                    [0, buf],
                                    [np.max(np.abs(p)), qubit.pulse.sigma],
                                    [0, buf],
                                    [0, len(d)],
                                    [0, tw],
                                    [0, len(d)]
        ...
        return cavity_dac_pulse, qubit_dac_pulse, cavity_dac_gauss, qubit_dac_gauss
    ...
    def ratios(alpha, tw):
        ...
        def cost(x):
            ...
            r1 = x[2]
            r2 = x[3]
            cavity_dac_pulse, qubit_dac_pulse, cavity_dac_gauss, qubit_dac_gauss = construct_CD(alpha, tw, r, r0, r1, r2)
    ...
    if curvature_correction:
        ...
        while np.abs(diff) / np.abs(beta) > 1e-3:
            ...
            r, r0, r1, r2 = ratios(alpha, tw)
            cavity_dac_pulse, qubit_dac_pulse, cavity_dac_gauss, qubit_dac_gauss = construct_CD(
    ...
    # between ECD pulses
    return  cavity_dac_pulse, qubit_dac_pulse, cavity_dac_gauss, qubit_dac_gauss , alpha, tw
...
def conditional_displacement_circuit(
    ...
    cavity_dac_gauss = []
    qubit_dac_gauss= [[] for _ in thetas]
    alphas = []
    tws = []
    cd_qubit_phases = []
...
#         ):  # if it's a disp at the end less than 1e-3, it won't matter anyway. Need to handle the pi pulse in this case...
        ...
        if (
            ...
            # here e_cd is cavity pulse and o_cd is qubit pulse
            e_cd, o_cd, e_cd_gauss, o_cd_gauss, alpha, tw = conditional_displacement(
                beta,
                alpha=alpha_CD,
                storage=storage,
                qubit=qubit,
...
#             cd_qubit_phases.append(0)
        ...
        for j in range(len(thetas)):
            ...
            o_r_gauss = np.array([[ qubit.pulse.unit_amp
                    * (theta / np.pi)
                    * np.max(pr)
                    * np.exp(1j * phi), qubit.pulse.sigma ]])

            # don't want to echo any qubit pulses yet 
            ...
            qubit_dac_gauss[j].append(np.array([np.exp(-1j * cumulative_qubit_phase) * o_r_gauss[0][0], o_r_gauss[0][1] ]))
            #cavity_dac_gauss.append(np.array([0, len(o_r)]))
            # print ('------')
            # print(i)
            # print(j)
            # print(np.exp(-1j * cumulative_qubit_phase) * o_r_gauss[0][0])
            
            if buffer_time > 0 and len(qubit_dac_pulse[0]) > 0:
                qubit_dac_pulse[j].append(np.zeros(buffer_time))
                qubit_dac_gauss[j].append(np.array([0, buffer_time]))
                #cavity_dac_gauss.append(np.array([0, buffer_time]))
            
            # qubit during ecd gate    
            qubit_dac_pulse[j].append(np.exp(-1j * cumulative_qubit_phase) * o_cd)
            for idx in range(len(o_cd_gauss)): 
                qubit_dac_gauss[j].append(np.array([np.exp(-1j * cumulative_qubit_phase) * o_cd_gauss[idx][0],
                                       o_cd_gauss[idx][1]]))
        ...
        cavity_dac_pulse.append(beta_sign * e_cd)   # beta sign not important since we are not echoing qubit pulses
        for idx in range(len(e_cd_gauss)): 
            cavity_dac_gauss.append(e_cd_gauss[idx])
    ...
    #qubit_dac_gauss = [np.concatenate(qp, axis = 0) for qp in qubit_dac_gauss]
    ...
    return_dict = {
        ...
        "cavity_dac_gauss": np.array(cavity_dac_gauss),
        "qubit_dac_gauss": np.array(qubit_dac_gauss[0]), # ********** Change this if multiple qubit rotations between ECDs
