...
#V9: WIth MECD code
def analysis(f_name):
    file = hf.File(fname, 'r')
    timestamps = list(file.keys())

    fidelities = list(file[timestamps[-1]]['fidelities']) # size: # of epochs x #of multistarts
    num_epochs = len(fidelities)
    num_multistarts = len(fidelities[0])

    # will transpose array  to rearrange for plotting 
    fidelities_t = np.transpose(fidelities)

    #rearrange fidelities
    # multistarts = np.zeroes(num_multistarts, num_epochs)
    # for i in range(num_epochs): 
    #     for j in range(num_multistarts): 
    #         multistarts[j][i] = fidelities  
    fig = plt.figure(figsize = (8,6))
    for i in range(num_multistarts): 
        plt.plot(1 - fidelities_t[i])
