#%%
#V2 - With support for saving data to h5 files
#V3 - clipping betas, gammas to be within betas, gammas scale
...
#         self.__init__(**parameters)
    ...
    def batch_construct_displacement_operators(self, alphas1, alphas2):
        '''
        WARNING: Scroll to bottom of this file; this function is overwritten!
...
#                         print(new_fids)
                    ...
                    self.betas_rho.assign(tf.clip_by_value(
                                    self.betas_rho, -beta_scale, beta_scale, name=None
                                )
                                )
                    self.gammas_rho.assign(tf.clip_by_value(
                                    self.gammas_rho, -gamma_scale, gamma_scale, name=None
                                )
                                )
                dfids = new_fids - fids
                fids = new_fids
...
class FixedBatchOptimizer(BatchOptimizer):
    ...
    def _construct_needed_matrices(self):
        ...
        self.a1 = self.tensor_product_baby_matrices(tfq.destroy(N_cav1), mode = 1)
        self.adag1 = self.tensor_product_baby_matrices(tfq.create(N_cav1), mode = 1)
        self.a2 = self.tensor_product_baby_matrices(tfq.destroy(N_cav2), mode = 2)
        self.adag2 = self.tensor_product_baby_matrices(tfq.create(N_cav2), mode = 2)
    ...
    def batch_construct_displacement_operators(self, alphas1, alphas2):
        ...
        Input: a list of displacements for 
        '''
        alphas1_star = tf.math.conj(alphas1)
        exponent = tf.einsum('ij,kl->ijkl', alphas1, self.adag1) - tf.einsum('ij,kl->ijkl', alphas1_star, self.a1)
        disp1s = tf.linalg.expm(exponent)

        alphas2_star = tf.math.conj(alphas2)
        exponent = tf.einsum('ij,kl->ijkl', alphas2, self.adag2) - tf.einsum('ij,kl->ijkl', alphas2_star, self.a2)
        disp2s = tf.linalg.expm(exponent)
