...
def conditional_displacement_circuit_ge(
    ...
    for l in range(N_layers): #construct each layer first 
        for m__ in range(N_modes): # currently assuming N_modes =1 
            ...
            cd_qubit_phases[0].append(analytic_dict["qubit_phase"][0])
            cd_qubit_phases[1].append(analytic_dict["qubit_phase"][1])   
            analytic_betas.append(analytic_dict["beta"])
    ...
    #             cd_qubit_phases.append(0)
            ...
            qubit_dac_pulse[1].append(np.exp(-1j * cumulative_qubit_phase[1]) * o_r_ef)
            
            qubit_dac_pulse[0].append(np.zeros(len(o_r_ef)))
            ...
            qubit_dac_pulse[0].append(np.exp(-1j * cumulative_qubit_phase[0]) * o_r_ge)
            
            qubit_dac_pulse[1].append(np.zeros(len(o_r_ge)))

            for m_ in range(N_modes): 
                cavity_dac_pulse[m_].append(np.zeros(len(o_r_ge) + len(o_r_ef)))  # qubit rotation happening, all modes on standby

            # print(np.shape(cavity_dac_pulse))
            # print(np.shape(qubit_dac_pulse[0]))
            # print(np.shape(qubit_dac_pulse[1]))
            # print(len(o_r_ef))
            # print(len(o_r_ge))
            # print(phi)
            # print(theta)
            ...
            #     qubit_dac_pulse[0].append(np.zeros(buffer_time))
            qubit_dac_pulse[0].append(np.zeros(buffer_time))
            qubit_dac_pulse[0].append(np.exp(-1j * cumulative_qubit_phase[0]) * o_cd)
            qubit_dac_pulse[0].append(np.zeros(buffer_time))
            ...
            # update the frame of the qubit
            cumulative_qubit_phase[0] += cd_qubit_phases[0][-1]
            cumulative_qubit_phase[1] += cd_qubit_phases[1][-1]
            #cavity_dac_pulse.append(np.zeros(wait_time))
...
def analytic_CD_ge(epsilon, Omega, chi):
    ...
    return {
        ...
        "correction": [correction_ge, correction_f],
        "theta_prime": [theta_prime_ge, theta_prime_f],
        #"qubit_phase": [theta_prime_ge, (theta_prime_f - theta_prime_ge)/2],
        "qubit_phase":[theta_prime_ge + (theta_prime_f/3),  (2*theta_prime_f/3)],
        "beta": beta,
    }
