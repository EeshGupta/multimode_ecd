...
class qutip_sim_two_mode:
    ...
    def add_cavity_dephasing(self, T1_mode1 = 10e+6, Techo_mode1 = 10e+6, T1_mode2 = 10e+6, Techo_mode2 = 10e+6 ):
        ...
        '''
        gamma_relax_mode1 = 1/T1_mode1
        gamma_echo_mode1 = 1/Techo_mode1
        gamma_phi_mode1 = gamma_echo_mode1 - (gamma_relax_mode1/2)

        gamma_relax_mode2 = 1/T1_mode2
        gamma_echo_mode2 = 1/Techo_mode2
        gamma_phi_mode2 = gamma_echo_mode2 - (gamma_relax_mode2/2)
        
        #assume both have the same max alphaCD (intermediate phase space radius)
        #alpha_CD = np.abs(max(self.alpha1))
        
        #approximating by using max value in alpha list (don't know how to input time dependent amplitudes on 
        # collapse operators)
        disp_frame_adag = self.adag_c + (np.abs(max(np.conjugate(self.alpha1)))*self.identity_c)
        disp_frame_a = self.a_c + (np.abs(max(self.alpha1))*self.identity_c)
        
        term1 = np.sqrt(gamma_phi_mode1)*tensor(self.identity_q, disp_frame_adag*disp_frame_a, self.identity_c)
        term2 = np.sqrt(gamma_phi_mode2)*tensor(self.identity_q, self.identity_c, disp_frame_adag*disp_frame_a)
