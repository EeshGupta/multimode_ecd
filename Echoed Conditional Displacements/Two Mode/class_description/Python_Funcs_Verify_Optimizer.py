...
class Calculator_gef_ECD(): 
    ...
    #     else: 
        ...
        if version == 'ge':
            mat = ( np.kron( basis(self.n_q, 1) * (basis(self.n_q, 0).dag()), d_adjoint) +
                    np.kron( basis(self.n_q, 0) * (basis(self.n_q, 1).dag()) , d) +
                    np.kron( basis(self.n_q, 2) * (basis(self.n_q, 2).dag()), self.identity_mm) 
                    )
       
        elif version == 'ef':
            mat = ( 
                    np.kron( basis(self.n_q, 0) * (basis(self.n_q, 0).dag()), self.identity_mm) +
                    np.kron( basis(self.n_q, 2) * (basis(self.n_q, 1).dag()), d_adjoint) +
                    np.kron( basis(self.n_q, 1) * (basis(self.n_q, 2).dag()) , d) 
                    )
        return Qobj(mat)

    
    def qubit_rot(self, phi, theta, version):
        '''
        Returns qubit rotation
        '''
        #
